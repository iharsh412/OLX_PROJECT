  ...
  async function onClickGoogle() {
    try {
      ...
      if (result?.user) {
        ...
        dispatch(updateAuthTokenRedux({ token: token, uId: result?.user?.uid ,userName:result?.user?.displayName}));
        navigate("/");
      } else {
        dispatch(updateAuthTokenRedux({ token: null, uId: null,userName:null}));
      }
    } catch (e) {
      ...
      <div className="loginPageChild">
        <ImageSection />
            ...
            >
              ...
              <span className="continue_with_google_text">Continue with Google</span>
            </button>
