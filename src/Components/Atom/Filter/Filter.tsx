...
const Filter: React.FC<FilterProps> = ({ category, response, sampleData, setSampleData, price, setPrice }) => {
    ...
    return (<div className={CLASSNAME.MAIN_SECTION_FILTER}>
                    ...
                    response?.subcategories?.map(
                        (
                            ...
                            <button
                                ...
                                key={index}
                                className={CLASSNAME.SUBCATEGORY_ITEM}
                                type={TYPE.BUTTON}
                                disabled={
                    ...
                    response?.Brand?.map(
                        (brand: string | undefined, index: number) => (
                            <button
                                ...
                                title={brand}
                                key={index}
                                className={CLASSNAME.BRAND_ITEMS}
        ...
        <div className={CLASSNAME.PRICE}>
            ...
            <div className={CLASSNAME.PRICE_LIST}>
                <input
                    ...
                    value={(price?.[0] ?? 0) < (price?.[1] ?? 0) ? (price?.[0] ?? 0) : (price?.[1] ?? 0)}
                    onChange={(e) => {
                ...
                <input
                    ...
                    value={(price?.[1] ?? 0) > (price?.[0] ?? 0) ? (price?.[1] ?? 0) : (price?.[0] ?? 0)}
                    onChange={(e) => {
                        const value = Number(e.target.value);
